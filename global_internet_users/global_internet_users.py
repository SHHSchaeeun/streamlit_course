...
if mnu == '설명':
    ...
    * **Cellular Subscription** - 인구 100명당 휴대폰 구독 수입니다. 평균적으로 한 사람이 모바일 서비스를 두 개 이상 구독하는 경우 이 수치는 100을 초과할 수 있습니다.
    * **Internet Users(%)** - 전 세계 모든 국가에서 인터넷에 접속하는 인구의 비율입니다.
    * **No. of Internet Users** - 모든 국가에서 인터넷을 사용하는 인구 수입니다.
    * **Broadband Subscription** - 인구 100명당 고정 광대역 가입 건수입니다. 이는 256kbit/s 이상의 다운스트림 속도로 공용 인터넷에 고속 액세스(TCP/IP 연결)하는 고정 구독을 의미합니다.
    ''')
    st.markdown('''
    **▍ 데이터 분석 목표**
    ...
    st.markdown('* world_df.info')
    world_df.info(buf=buffer)
    st.text(buffer.getvalue())

    buffer.truncate(0)

    st.markdown('* region_df.info()')
    region_df.info(buf=buffer)
    st.text(buffer.getvalue())

    buffer.truncate(0)

    st.markdown('* income_df.info()')
