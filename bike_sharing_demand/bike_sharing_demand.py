...
''')
    ...
    st.code("all_data = pd.concat([train, test], ignore_index=True)")
    ...
    st.write('훈련데이터는 매달 1일부터 19일까지의 기록이고, 테스트 데이터는 매달 20일부터 월말까지의 기록이다.'
             '그러므로 대여 수량을 예측할 때 일(day) 피처는 사용할 필요가 없다.'
             'minute와 second 피처도 모든 기록이 같은 값이므로 예측에 사용할 필요가 없다.')
    ...
    st.markdown('**피처 선택이란?**')
    st.markdown('모델링 시 데이터의 특징을 잘 나타내는 주요 피처만 선택하는 작업을 **피처 선택(feature selection)**.')
    ...
    st.write('[위키디피아: 선형회귀](https://ko.wikipedia.org/wiki/%EC%84%A0%ED%98%95_%ED%9A%8C%EA%B7%80)')
    ...
    st.code('''
    from sklearn.linear_model import LinearRegression # scikit-learn 설치
