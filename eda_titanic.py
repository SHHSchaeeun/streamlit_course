...
if mnu == '타이타닉 분석':
    ...
    st.dataframe(titanic[['sex', 'survived']].groupby(['sex'], as_index=True).mean().sort_values(by='sex', ascending=False))
    ...
    g.map(plt.hist, 'fare', bins=20, color='#B0CBC4')
    st.pyplot()
    ...
    st.text('생존 여부(survived), 등급에 따른 나이(age) 분포')
